...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
	...
	--adding all the threads onto freeList	
	...
	if (freeList.IsEmpty())
		...
		while(!tempThread)
			...
			endIf			
			--when mutex lock is reaquired, proceed as normal
			if (threadManagerLock.IsHeldByCurrentThread())
				tempThread = freeList.Remove()
			endIf
			
		endWhile
	--if freeList is not empty, take a thread off freeList
	else
		tempThread = freeList.Remove()		
	endIf
	--change the status of the thread
	tempThread.status = JUST_CREATED
	--release mutex lock
	...
	--aquire mutex lock
	threadManagerLock.Lock()
	--change status, add to end of freeList
	th.status = UNUSED
	freeList.AddToEnd(th)	
	--signal that there is a free thread
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
	...
	--initializing each ProcessControlBlock
	--default status is set to FREE
	...
	--initialize condition even though it is not used
	...
	aProcessDied.Init()
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        --
	var
	tempProcess: ptr to ProcessControlBlock
	--aquire mutex lock
	processManagerLock.Lock()
	--if freeList is empty
	if (freeList.IsEmpty())
		--while tempProcess has not yet been assigned
		while(!tempProcess)
			--wait on condition
			if(!tempProcess)
				aProcessDied.Wait(&processManagerLock)
			endIf
			--if the lock has been required
			if (processManagerLock.IsHeldByCurrentThread())
				--Remove from freeList
				tempProcess = freeList.Remove()
			endIf
		endWhile
	...
	nextPid = nextPid + 1
	tempProcess.pid = nextPid
	tempProcess.status = ACTIVE
	...
	return tempProcess
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- to the FREE list.
        --
	-- essentially the same as freeThread
	...
	for i = 0 to MAX_NUMBER_OF_PROCESSES-1
		if p.parentsPid == processTable[i].pid
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
	...
	var
	i, temp, frameAddr: int
	--obtains the mutex lock 
	frameManagerLock.Lock()
	...
	for i = 0 to numFramesNeeded - 1
		temp = framesInUse.FindZeroAndSet()	
		frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (temp * PAGE_SIZE)	
	...
	var 
	numFramesReturned, i, frameAddr, bitNumber: int
...
function Handle_Sys_Fork () returns int
	...
	var 
		...
		pcb: ptr to ProcessControlBlock
		oldIntStatus, numFrames, i, oldUserPC: int	
		currentPcb: ptr to ProcessControlBlock	

	newThrd = threadManager.GetANewThread()

	currentPcb = currentThread.myProcess
	
	pcb = processManager.GetANewProcess()
	pcb.myThread = newThrd
	pcb.parentsPid = currentPcb.pid

	newThrd.myProcess = pcb
	...
	for i = 0 to MAX_NUMBER_OF_OPEN_FILES - 1
		pcb.fileDescriptor[i] = currentPcb.fileDescriptor[i]
		if pcb.fileDescriptor[i] != null
			pcb.fileDescriptor[i].numberOfUsers = pcb.fileDescriptor[i].numberOfUsers + 1
		endIf
	endFor
	fileManager.fileManagerLock.Unlock()

	numFrames = currentPcb.addrSpace.numberOfPages
	frameManager.GetNewFrames(&pcb.addrSpace, numFrames)

	for i = 0 to numFrames - 1
		MemoryCopy(pcb.addrSpace.ExtractFrameAddr(i),
                  currentPcb.addrSpace.ExtractFrameAddr(i), PAGE_SIZE)	

		if currentPcb.addrSpace.IsWritable(i)
...
-----------------------------  Handle_Sys_Join  ---------------------------------
	...
	for i = 0 to MAX_NUMBER_OF_PROCESSES-1
		...
		if processID == child.pid && currentThread.myProcess.pid ==  child.parentsPid && child.status != FREE
			temp = processManager.WaitForZombie(child)
			return temp
		endIf
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
	...
	var 
		...
		execFile = fileManager.Open(&strBuffer)
		if execFile == null
			return -1
		endIf
		...
		startingPoint = execFile.LoadExecutable(&newAddrSpace)

		if startingPoint == -1
			return -1
		endIf		

		fileManager.Close(execFile)
		...
		UserStack = currentThread.myProcess.addrSpace.numberOfPages * PAGE_SIZE
		SystemStack = (&currentThread.systemStack[SYSTEM_STACK_SIZE-1]) asInteger 

		currentThread.myProcess.addrSpace.SetToThisPageTable()
		currentThread.isUserThread = true
		BecomeUserThread(UserStack, startingPoint, SystemStack)
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
	...
	ret = pcb.addrSpace.GetStringFromVirtual(&strBuffer, filename asInteger, MAX_STRING_SIZE)
	...
	for i = 0 to MAX_FILES_PER_PROCESS - 1
		if pcb.fileDescriptor[i] == null
			pcb.fileDescriptor[i] = fileManager.Open(&strBuffer)
			if pcb.fileDescriptor[i] == null
				return -1
			else
				return i
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
	...
	if (fileDesc < 0 || fileDesc > MAX_FILES_PER_PROCESS || pcb.fileDescriptor[fileDesc] == null || sizeInBytes < 0) 
		return -1
	else 
		file = pcb.fileDescriptor[fileDesc]
	endIf
	...
	while true
		...
		if nextPosInFile + thisChunkSize > file.fcb.sizeOfFileInBytes
			thisChunkSize = file.fcb.sizeOfFileInBytes - nextPosInFile
		endIf
		...
		if (virtPage < 0 || virtPage > NUMBER_OF_PHYSICAL_PAGE_FRAMES || !(pcb.addrSpace.IsValid(virtPage)) || !(pcb.addrSpace.IsWritable(virtPage)))
			return -1
		endIf
		
		pcb.addrSpace.SetDirty(virtPage)
		pcb.addrSpace.SetReferenced(virtPage)
		destAddr = pcb.addrSpace.ExtractFrameAddr(virtPage) + offset

		if destAddr == 0
			return copiedSoFar
		endIf
		
		temp = fileManager.SynchRead(file, destAddr, nextPosInFile, thisChunkSize) 
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
	...
	while true
		...
		pcb.addrSpace.SetReferenced(virtPage)
		destAddr = pcb.addrSpace.ExtractFrameAddr(virtPage) + offset
		temp = fileManager.SynchWrite(file, destAddr, nextPosInFile, thisChunkSize) 
...
-----------------------------  Handle_Sys_Seek  ---------------------------------
  ...
  function Handle_Sys_Seek (fileDesc: int, newCurrentPos: int) returns int
	...
	if (fileDesc < 0 || pcb.fileDescriptor[fileDesc] == null || newCurrentPos < -1 || newCurrentPos > pcb.fileDescriptor[fileDesc].fcb.sizeOfFileInBytes) 
		fileManager.fileManagerLock.Unlock()
		return -1
	else 
		file = pcb.fileDescriptor[fileDesc]
	...
	if newCurrentPos == -1
		file.currentPos = file.fcb.sizeOfFileInBytes
		fileManager.fileManagerLock.Unlock()
...
-----------------------------  Handle_Sys_Close  ---------------------------------
  ...
  function Handle_Sys_Close (fileDesc: int)
	...
	if (fileDesc > MAX_FILES_PER_PROCESS || fileDesc < 0 || pcb.fileDescriptor[fileDesc] == null) 
		return
	else
		fileManager.Close(pcb.fileDescriptor[fileDesc])
		pcb.fileDescriptor[fileDesc] = null
	endIf 	
...
function InitFirstProcess()
	...
	temp = threadManager.GetANewThread()
	temp.Init("UserProgram")
	temp.Fork(StartUserProcess,0)	
...
-----------------------------  StartUserProcess  ---------------------------------
	...
	temp = processManager.GetANewProcess()
	temp.myThread = currentThread
	currentThread.myProcess = temp

	execFile = fileManager.Open("TestProgram4")
	startingPoint = execFile.LoadExecutable(&temp.addrSpace)
	fileManager.Close(execFile)

	userStack = temp.addrSpace.numberOfPages * PAGE_SIZE
	systemStack = (&currentThread.systemStack[SYSTEM_STACK_SIZE-1]) asInteger 

	oldIntStatus = SetInterruptsTo (DISABLED)
	temp.addrSpace.SetToThisPageTable()
	currentThread.isUserThread = true
	BecomeUserThread(userStack, startingPoint, systemStack)	
...
----------------------------- ResumeChildAfterFork --------------------------------
	...
	userStack = currentThread.userRegs[14] --stack pointer
	systemStack = (&currentThread.systemStack[SYSTEM_STACK_SIZE-1]) asInteger 
	BecomeUserThread(userStack, oldUserPC, systemStack)
