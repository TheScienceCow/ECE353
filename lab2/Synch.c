...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
	 ...
	 if heldBy == null	--if not locked
	 heldBy = currentThread --lock is on current thread
	 else
	   waitingThreads.AddToEnd (currentThread) --if already locked, add thread to waiting list
	     currentThread.Sleep()			-- put thread to sleep
	 endIf 
          oldIntStat = SetInterruptsTo (oldIntStat)  --returning to old interrupt condition
        endMethod
	 ...
	 var
            ...
            oldIntStat: int
          oldIntStat = SetInterruptsTo (DISABLED)   --disabling interrupts
	    if heldBy == currentThread			--only the current thread can unlock
		heldBy = null				--unlock if empty list
	    if !(waitingThreads.IsEmpty())		--if not empty
            t = waitingThreads.Remove () 		--pop the first thread 
            t.status = READY				--set it to ready status
            readyList.AddToEnd (t)			--add it to the end of the readylist
	    heldBy = t					--put lock on next thread (may be null)
	    endIf
	    endIf
          oldIntStat = SetInterruptsTo (oldIntStat)  --returning to old interrupt condition
         endMethod
